...
test_that("get histogram works", {
	...
	hist <- ds$getHistogram(1, -0.5, 100.5, 101, FALSE, FALSE)
	expect_equal(length(hist), 101)
	expect_equal(sum(hist), 15301)
	ds$close()
	deleteDataset(f)
	...
	expect_true(ds$setDefaultHistogram(1, -0.5, 100.5, 101, hist))
	expect_no_warning(ds$getDefaultHistogram(1, FALSE))
	expect_length(ds$getDefaultHistogram(1, FALSE), 4)
	ds$close()
	deleteDataset(f2)
})
