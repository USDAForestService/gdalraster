...
The function `displayRAT()` generates a presentation table. Colors are shown if the RAT contains RGB columns:

```{r}
displayRAT(rat2, title = "Raster Attribute Table for Storm Lake EVT")
```

`displayRAT()` returns an object of class `"gt_tbl"`. The table object could be modified if desired using functions of the **gt** package.

## Using in QGIS

[QGIS](https://qgis.org/) since 3.30 includes [extensive support for Raster Attribute Tables](https://qgis.org/en/site/forusers/visualchangelog330/index.html#feature-raster-attribute-tables-rat-suppport).
...
* Package [gt](https://gt.rstudio.com/)
