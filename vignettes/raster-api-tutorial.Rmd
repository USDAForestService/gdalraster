...
An error is returned if the dataset cannot be opened (and creation of the `GDALRaster` object fails). Also, note that `filename` may not actually be the name of a physical file (though it usually is). Its interpretation is driver dependent, and it might be a URL, a database connection string, a filename with additional parameters, and so forth.
...
In the common case of a "north up" raster without any rotation or shearing, the georeferencing transform (see [Geotransform Tutorial](https://gdal.org/tutorials/geotransforms_tut.html) takes the following form with 1-based indexing in R:
...
In the general case, this is an affine transform. `GDALRaster` also provides the follow convenience methods for the case of a north-up raster:
...
# raster size in pixels X, pixels Y, number of bands
ds$getRasterXSize()
ds$getRasterYSize()
ds$getRasterCount()
...
Writing data with `GDALRaster$write()` is similar to `read()` with an additional argument specifying a vector of pixel data to write. The `xoff`, `yoff`, `xsize`, `ysize` arguments describe the window of raster data on disk to read (or write). It doesn't have to fall on tile boundaries though access may be more efficient if it does (in some cases). Note that GDAL uses memory caching algorithms during raster I/O to improve performance. The operation of the caching mechanism and potential configuration of cache memory size should be considered when scaling I/O to large datasets (see [GDAL Block Cache](https://usdaforestservice.github.io/gdalraster/articles/gdal-block-cache.html)).

The `out_xsize` and `out_ysize` values describe the size of the input buffer (an R vector of length `out_xsize` x `out_ysize` that data will be read into). When loading data at full resolution this would be the same as the window size (`xsize`, `ysize`). However, to load a reduced resolution overview this could be set to smaller than the window on disk. In this case `read()` will utilize overviews to do the I/O more efficiently if the overviews are suitable.
...
To determine if a particular format supports Create or CreateCopy, consult the table of raster format drivers at https://gdal.org/drivers/raster/index.html. Note that a number of drivers are read-only and won't support Create or CreateCopy.

## Using createCopy()

`gdalraster::createCopy()` can be used fairly simply as most information is collected from the source file. It includes an argument for passing format specific creation options. The following code copies a multi-band raster in FARSITE v.4 LCP format (basically a raw format without support for compression or nodata value) to LZW-compressed GTiff:
...
For situations in which you are not just exporting an existing file to a new file, it is generally necessary to use  `gdalraster::create()`. This function takes an options list much like `createCopy()`, but the raster size, number of bands and band type must be provided explicitly:
