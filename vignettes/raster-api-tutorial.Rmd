...
The `out_xsize` and `out_ysize` values describe the size of the buffer (an R vector of length `out_xsize * out_ysize` that data will be read into). When loading data at full resolution this would be the same as the window size (`xsize`, `ysize`). However, to load a reduced resolution overview this could be set to smaller than the window on disk. In this case `read()` will utilize overviews to do the I/O more efficiently if the overviews are suitable.
