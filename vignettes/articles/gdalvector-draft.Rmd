...
* existing utility functions for managing vector data sources (added at **gdalraster** 1.10.0.9180 dev): [ogr_manage](https://usdaforestservice.github.io/gdalraster/reference/ogr_manage.html) and [ogr_define](https://usdaforestservice.github.io/gdalraster/reference/ogr_define.html)
* existing wrappers `ogr2ogr()` and `ogrinfo()` from the GDAL Utils API (**gdalraster** 1.10.0 CRAN release)
...
An OGR Feature &ndash; as read by `GDALVector::getNextFeature()` or `GDALVector::getFeature()`, or as passed to write methods &ndash; is a list with the unique feature identifier (FID), attribute and geometry field names, and their values. `GDALVector::getFeatureSet()` will return a set of features as a data frame.
...
<sup>Created on 2024-04-06 with [reprex v2.1.0](https://reprex.tidyverse.org)</sup>
...
<sup>Created on 2024-05-14 with [reprex v2.1.0](https://reprex.tidyverse.org)</sup>
...
* handling of 64-bit integer: OGR FID currently is `GIntBig` and integer fields in vector data sources will commonly be `OFTInteger64`. These are now handled using **RcppInt64**, with **bit64** on the R side providing S3 class `integer64`.
