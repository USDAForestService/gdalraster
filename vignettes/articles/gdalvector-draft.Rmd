...
An OGR Feature, as read by `GDALVector::getNextFeature()` or as passed to write methods, is a list with the unique feature identifier (FID), attribute and geometry field names, and their values. `GDALVector::getFeatureSet()` will return a set of features as a data frame. (Note that FID is an `int64` type in GDAL. Handling of `int64` TBD, currently as R `double` type).
