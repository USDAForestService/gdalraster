...
* `ogr_execute_sql()`: SQL `SELECT` statements can be used to open a layer as an object of class `GDALVector` described below. `ogr_execute_sql()` will execute an SQL statement against the data store for `CREATE INDEX`, `DROP INDEX`, `ALTER TABLE`, `DROP TABLE`.
