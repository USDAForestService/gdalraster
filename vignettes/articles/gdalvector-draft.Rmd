...
**gdalraster** returns OGR field types as corresponding native R types (`NA` for OGR NULL values):
* `OFTInteger`: `integer` (`OFSTBoolean` subtype as `logical`)
* `OFTIntegerList`: vector of `integer` (list column in data frames)
* `OFTInteger64`: `bit64::integer64` (`OFSTBoolean` subtype as `logical`)
