...
* existing utility functions for managing vector data sources (added at **gdalraster** 1.11.0 release): [ogr_manage](https://usdaforestservice.github.io/gdalraster/reference/ogr_manage.html) and [ogr_define](https://usdaforestservice.github.io/gdalraster/reference/ogr_define.html)
* existing wrappers `ogr2ogr()` and `ogrinfo()` from the GDAL Utils API (**gdalraster** 1.10.0 release)
...
An OGR Feature &ndash; as read by `GDALVector::getNextFeature()` or `GDALVector::getFeature()`, or as passed to write methods &ndash; is a list with the unique feature identifier (FID), attribute and geometry field names, and their values. `GDALVector::fetch()` fetches the next n features from the layer and returms them as a data frame (analog of [`DBI::dbFetch()`](https://dbi.r-dbi.org/reference/dbFetch.html)).
