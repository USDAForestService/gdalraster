...
Tests were run on a Dell XPS 13 laptop with Intel(R) Core(TM) i5-8250U CPU @ 1.60GHz and SSD, Ubuntu 22.04.2, R 4.3.0, gdalraster 1.2.1 (dev), GDAL 3.6.2.
...
```
## in MB
gdalraster::set_config_option("GDAL_CACHEMAX", "500")

## or percent of usable physical RAM
gdalraster::set_config_option("GDAL_CACHEMAX", "10%")
```

I/O that involves block caching with large datasets may require setting `GDAL_CACHEMAX` larger than the default 5% of usable physical RAM. If the LANDFIRE elevation raster were tiled at 256 x 256, then each block would require 65536 x 2 = 131072 bytes for 16-bit data. The cache size needed to hold all intersected blocks for a row would be approximately 160 MB. Similarly, the cache size could be configured for the case of multiple large rasters that need to be read simultaneously for processing.
