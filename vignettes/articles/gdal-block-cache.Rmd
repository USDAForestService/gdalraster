...
It is also worth noting that without the block caching mechanism, it is not possible to read the tiled elevation raster by row in reasonable time. This can be checked by repeating Test 1 above with the block cache disabled:

```
## Test 4
## original tiled raster, reading by row (across block boundaries)
## block cache disabled for testing only!
