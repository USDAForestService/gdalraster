...
Core functionality for vector is provided by an exposed C++ class `GDALVector`, along with a set of stand-along functions with the `ogr_` prefix.
...
* fetch the full feature set and return as data frame
* fetch progressively in batches of `n` features at a time
* read via column-oriented Arrow ArrayStream
...
OGR facilities for vector geoprocessing are available in `ogr_proc()`. This function performs the following GIS overlay operations: Intersection, Union, SymDifference, Identity, Update, Clip and Erase (https://en.wikipedia.org/wiki/Vector_overlay). `ogr_proc()` is basically an R port of the command-line utility [`ogr_layer_algebra`](https://gdal.org/en/stable/programs/ogr_layer_algebra.html#ogr-layer-algebra) which is included in the GDAL Python bindings. Both of these are interfaces to library functions in the OGR C++ API.

`ogrinfo()` and `ogr2ogr()`, provide R wrappers of the GDAL command-line utilities [`ogrinfo`](https://gdal.org/en/stable/programs/ogrinfo.html) and [`ogr2ogr`](https://gdal.org/en/stable/programs/ogr2ogr.html). These functions support all of the command-line arguments described in the GDAL documentation, providing a powerful set of capabilities for obtaining information about an OGR-supported data source, converting vector data between file formats, and potentially editing data with SQL statements. `ogr_reproject()` is a convenience wrapper around `ogr2ogr()` for reprojecting vector layers with a user friendly interface and comprehensive documentation.

### Supporting APIs

#### Spatial Reference Systems

Bindings to a subset of GDAL's Spatial Reference System API are provided by a set of stand-alone `srs_*()` functions. These support conversion of spatial reference definitions in various formats to OGC Well Known Text (WKT) representation (e.g., EPSG codes, well known names such as NAD27, NAD83, WGS84, etc., PROJ.4 definitions, PROJJSON, and others, as well as between different versions of OGC WKT, see `?srs_convert`). Functions under the `?srs_query` topic provide various information about a given SRS definition, and support testing definitions for equality.
...
Note that parentheses around statements are used throughout the examples only as a shortcut to display the assigned values. The `system.file()` command is only used to obtain the path to the package sample files; it would not be needed in typical usage.
...
The park boundary layer contains a single polygon feature. The list object returned for an individual feature carries the `"OGRFeature"` class attribute providing S3 methods for `print()` and `plot()`.
...
The following example introduces another sample dataset which contains fire perimeters for Yellowstone National Park from the Monitoring Trends in Burn Severity (MTBS) project. Here we set up a temporary writable copy of ynp_fires_1984_2022.gpkg (in memory), and use it as a data source to perform further processing.

The MTBS layer uses a projected coordinate system, while the layers in the "YNP features" dataset used above are in geographic coordinates. We use the `ogr_reproject()` function to project layers from ynp_features.gpkg to match the coordinate system of the MTBS layer.
...
```{r}
# add the YNP centroid as a point of interest
...
```{r}
# rewrite a point_of_interest feature to update the feature name
...
```{r}
# delete the YNP centroid feature the was created above
...
The `$batchCreateFeature()`method of a `GDALVector` object is batch version of `$createFeature()`. It creates and writes a batch of new features within the layer from input passed as a data frame. Column names in the data frame must match field names of the layer and have compatible data types.
...
By default, the GeoPackage driver automatically creates a column named `fid` to use for the OGR FID (primary key in the SQLite database). This can be configured with the `FID=value` layer creation option. Note that FID (feature id) is a special property of a feature and not treated as an attribute, so we do not specify it in the layer definition.
...
Here we generate a GeoJSON string containing a single polygon feature that defines a rectangular area of interest around the Maple Fire boundary. This format is supported for input by some data distribution systems to define a download box (e.g., [NLCD](https://www.mrlc.gov/viewer/) and [LANDFIRE](https://www.landfire.gov/viewer/)). For this case we need the coordinates in Web Mercator projection (EPSG 3857).

```{r}
# write the Maple Fire AOI bounding box as GeoJSON in EPSG 3857
json_file <- file.path(tempdir(), "maple_fire_aoi.geojson")

lyr <- ogr_ds_create("GeoJSON", json_file, "maple_fire_aoi",
                     geom_type = "POLYGON", srs = "EPSG:3857",
                     fld_name = "id", fld_type = "OFTString",
                     overwrite = TRUE, return_obj = TRUE)

# The Maple Fire polygon feature and spatial reference were obtained above in
# the section on "Reproject vector layers".
# We extend the minimum bounding box by 500 m in each direction.
feat <- list()
